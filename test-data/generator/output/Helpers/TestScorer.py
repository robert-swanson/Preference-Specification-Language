...
def graph(title, low, high, worstBound=None, bestBound=None, lowerQuartile=None, higherQuartile=None):
    ...
    y = []
    for xval in x:
        y.append(scorer.scoreSigmoid(xval, 1, title, diagnostics=True, worstBound=worstBound, bestBound=bestBound, lowerQuartile=lowerQuartile, higherQuartile=higherQuartile))
    plt.plot(x, y)
    plt.title(title)
    plt.show()
...
def graphOptimum(title, low, high, optimum, worstDeviance=None, lowerQuartileDeviance=None, leftWorstDeviance=None, leftLowerQuartileDeviance=None, rightWorstDeviance=None, rightLowerQuartileDeviance=None):
    ...
    y = []
    for xval in x:
        y.append(scorer.scoreOptimum(xval, 1, title, optimum, diagnostics=True, worstDeviance=worstDeviance, lowerQuartileDeviance=lowerQuartileDeviance, leftLowerQuartileDeviance=leftLowerQuartileDeviance, leftWorstDeviance=leftWorstDeviance, rightLowerQuartileDeviance=rightLowerQuartileDeviance, rightWorstDeviance=rightWorstDeviance))
    plt.plot(x, y)
    plt.title(title)
    plt.show()
...
def graphBoolean(title, low, high):
    ...
    y = []
    for xval in x:
        y.append(scorer.scoreBoolean(xval>0, 1, title, diagnostics=True))
    plt.plot(x, y)
    plt.title(title)
    plt.show()
